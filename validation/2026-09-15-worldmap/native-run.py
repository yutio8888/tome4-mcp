...
class Wire:
    ...
    def packet(self, op: str, args: dict) -> dict:
        self.sequence += 1
        return dict(v=1, id=f"r-{self.sequence}", op=op, args=args)
    ...
    def receive(self, packet: dict) -> dict:
        ...
        assert response["v"] == 1 and response["id"] == packet["id"], response
        return response
...
class Acceptance:
    ...
    def run(self) -> None:
        ...
        unsupported = self.wire.raw("act", self.action_args({"type": "use_talent", "talent_id": "T_PHASE_DOOR"}))
        self.check(not unsupported.get("ok") and unsupported["error"]["code"] == "unsupported_talent",
                   "unadapted_talent_rejected_before_execution")
    ...
    def run_mcp(self, python: Path) -> None:
        ...
        environment.update(PYTHONPATH=str(WORKSPACE / "tools/tome-mcp-server/src"),
