...
class BridgeClient:
    ...
    async def connect(self, mode: str = "control", protocol_version: int = 1) -> dict[str, Any]:
        ...
        if type(protocol_version) is not int or protocol_version not in {1, 2}:
            raise BridgeError("invalid_protocol_version", "Choose Bridge protocol 1 or 2.")
        async with self._lock:
    ...
    async def respond(self, args: dict[str, Any], *, wait_ms: int = 2000) -> dict[str, Any]:
        ...
        if self.protocol_version != 2:
            raise BridgeError("protocol_version_required", "Connect explicitly with protocol_version=2.",
                              command_id=args.get("command_id"), response_id=args.get("response_id"))
