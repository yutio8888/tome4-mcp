...
Action = Annotated[
    ...
    | SpendStatAction | LearnTalentAction | LearnCategoryAction | PickupAction | EquipAction | UnequipAction | UseItemAction,
    Field(discriminator="type"),
]
...
RULES = """ToME MCP Bridge protocols 1 and 2
Protocol 2 supports use_item with an owned item_id through native player item
...
Connect with protocol_version=2 explicitly for native interactive talent execution.
The default protocol 1 keeps its legacy audited talent adapters and target_id.
In protocol 2, use_talent takes only talent_id; do not prefill target_id. A talent
can produce zero, one, or several input requests. awaiting_input is a pause, not
...
explicit protocol-2 control connection if no manual handoff/context change occurred.
...
control="battle_companion" and phase="unavailable" mean local combat owns actions.
...
are terminal. In protocol 2, awaiting_input and running_native_task are pending;
...
These are committed native changes, not a reversible build preview. Learning a
...
Check capabilities and each current interaction. Protocol 2 automates audited
native target, confirmation, list and inventory-selection prompts from the active
talent, plus bounded native rest tasks. UI outside these providers, respecs and
character creation require manual interaction. Inventory choices reflect the
...
def create_server(bridge: BridgeClient) -> MCPServer:
    ...
    async def connect(mode: Literal["control", "observe"] = "control",
                      protocol_version: Literal[1, 2] = 1) -> ToolReply:
        ...
        try:
            return ToolReply(ok=True, result=await bridge.connect(mode, protocol_version))
        except BridgeError as exc:
            return ToolReply(ok=False, error=exc.as_dict())
    ...
    async def inspect(session_id: Identifier, kind: Literal["talent", "actor", "progression", "item"], id: Identifier) -> ToolReply:
        """Inspect a learned talent, visible actor, owned/visible item by its returned ID, or the progression tree with kind=progression and id=player. Reads never evaluate dynamic talent descriptions or identify objects. Unavailable details remain unknown."""
        return await call("inspect", {"session_id": session_id, "kind": kind, "id": id})
    ...
    async def respond(session_id: Identifier, control_token: Identifier, command_id: Identifier,
        ...
        """Protocol 2: answer the current native interaction exactly once. Keep the original command_id and a unique response_id. Wait for the next input or command result; on uncertainty query those IDs, never repeat the talent. Cancel preserves native cancellation semantics and can produce further effects or questions."""
