...
-- Interchangeable groups: the closed `group`/`equiv` vocabulary -------------
-- The maintainer chose option A (R2): the three Earthen Missiles prompts are
-- indistinguishable by curated signature yet semantically equivalent (one
-- computed `damage`, the same projectile and the same `DamageType.SPLIT_BLEED`
-- for every prompt; spells/stone.lua:40-56), so refusing them violates "a
-- faithfully executable legal action must not be refused". The mechanism is
-- NARROW and declared (never an identity/digest/closure gate).
do
    local PROOF='one computed damage and the same projectile/DamageType.SPLIT_BLEED '
        ..'for every prompt (spells/stone.lua:40-56)'
    local SIG={cursor_type='bolt',nowarning=true}
    ...
    local function invalid(entries,detail)
        ...
        return out==nil and err.reason=='movement_adapter_invalid' and err.detail==detail,err
    end
    -- The keys are closed: an unknown entry key is still rejected.
    local unknownKey,unknownErr=seq({{index=1,request='grid',subject='self',
        observed=SIG,group=1,equiv=PROOF,bogus=1}})
    check(unknownKey==nil and unknownErr.detail=='unknown_request_key'
        and unknownErr.key=='bogus','an unknown entry key is still rejected')
    -- >=2 members: a lone grouped entry is malformed (there is no group of one).
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
        observed=SIG,group=1,equiv=PROOF}},'group_too_small'),
        'a one-member group is movement_adapter_invalid')
    -- Same `request` kind across the group.
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=7,equiv=PROOF},
        {index=2,request='actor',subject='self',observed=SIG,group=7,equiv=PROOF}},
        'group_kind_mismatch'),'a group mixing request kinds is movement_adapter_invalid')
    -- The curated equivalence proof is required on EVERY member, must be a
    -- bounded string, and must cite a reviewed source location.
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=2},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=2,equiv=PROOF}},'missing_equivalence_proof'),
        'a group member without a curated proof is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=2,equiv=true},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=2,equiv=true}},'bad_equivalence_proof'),
        'a non-string proof is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=2,equiv='because'},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=2,equiv='because'}},'bad_equivalence_proof'),
        'a proof without a `.lua:<line>` citation is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
        observed=SIG,equiv=PROOF}},'equivalence_proof_without_group'),
        'a proof without a group is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=0,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=0,equiv=PROOF}},'bad_group'),'an out-of-range numeric group id is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group='Not Closed',equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group='Not Closed',equiv=PROOF}},'bad_group'),
        'a malformed string group id is rejected')
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=4,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=4,equiv='a different claim (spells/fire.lua:1)'}},'group_proof_mismatch'),
        'members whose curated proofs disagree are rejected')
    -- A WELL-FORMED group is admitted even though every signature is IDENTICAL
    -- (that is the point: order is interchangeable by curation).
    local grouped=assert(seq({
        {index=1,request='grid',subject='self',value_source='target_plan',observed=SIG,group=1,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,group=1,equiv=PROOF}}))
    check(#grouped.request_sequence==2 and grouped.target_requests[1]=='grid'
        and grouped.target_requests[2]=='grid','a two-member group expands')
    check(grouped.request_sequence[1].group==1 and grouped.request_sequence[1].equiv==PROOF,
        'the normalised entry keeps its declared group and proof')
    -- Invariant: an UNGROUPED subsumed pair keeps today's behaviour exactly.
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',observed=SIG},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG}},
        'request_signature_ambiguous'),
        'a subsumed UNGROUPED pair still yields request_signature_ambiguous')
    -- Invariant: a grouped entry against an UNGROUPED identical-signature entry
    -- is still refused (the exemption is exactly one declared group). The pair
    -- is a valid 2-member group; the third, ungrouped entry repeats the same
    -- signature, so both cross-group pairs are subsumptions and the descriptor
    -- is refused.
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=1,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=1,equiv=PROOF},
        {index=3,request='grid',subject='self',value_source='target_plan',observed=SIG}},
        'request_signature_ambiguous'),
        'a grouped entry against an ungrouped identical entry is still ambiguous')
    -- A lone grouped entry is caught earlier and more specifically
    -- (`group_too_small`); it can never reach the subsumption rule.
    local lone,loneErr=seq({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=1,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG}})
    check(lone==nil and loneErr.detail=='group_too_small',
        'a group of one is group_too_small, before any signature comparison')
    -- Two DISJOINT groups of >=2 members each, with identical signatures, are
    -- also refused (only SAME-group pairs are exempt).
    check(invalid({{index=1,request='grid',subject='self',value_source='target_plan',
            observed=SIG,group=1,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=1,equiv=PROOF},
        {index=3,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=2,equiv=PROOF},
        {index=4,request='grid',subject='self',value_source='target_plan',observed=SIG,
            group=2,equiv=PROOF}},'request_signature_ambiguous'),
        'two disjoint groups with identical signatures are still ambiguous')
    -- A TL5-shaped group of 3 (the third bonus missile) is admitted.
    local three=assert(seq({
        {index=1,request='grid',subject='self',value_source='target_plan',observed=SIG,group=1,equiv=PROOF},
        {index=2,request='grid',subject='self',value_source='target_plan',observed=SIG,group=1,equiv=PROOF},
        {index=3,request='grid',subject='self',value_source='target_plan',observed=SIG,group=1,equiv=PROOF}}))
    check(#three.request_sequence==3,'a three-member group (TL5 bonus projectile) expands')
    -- The runtime helper: members of `index`'s group in arrival order; an
    -- ungrouped entry is its own single-member set.
    check(table.concat(Factory.groupMembers(grouped.request_sequence,1),',')=='1,2',
        'groupMembers returns the same-group indexes in arrival order')
    check(table.concat(Factory.groupMembers(three.request_sequence,3),',')=='1,2,3',
        'groupMembers returns every member for any member index')
    check(#Factory.groupMembers({{index=1,request='grid',subject='self',observed=SIG}},1)==1,
        'an ungrouped entry is a single-member set')
    check(Factory.sameGroup({group=1},{group=1})==true
        and Factory.sameGroup({group=1},{group=2})==false
        and Factory.sameGroup({group=1},{})==false
        and Factory.sameGroup({},{group=1})==false,
        'sameGroup never matches an absent group')
end
