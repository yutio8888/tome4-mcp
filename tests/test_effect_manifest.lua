...
check(unsupportedEntry('T_SKIRMISHER_VAULT')==nil,'Vault is admitted, not unsupported')
    ...
    -- The dwarven specs carry friendlyfire=false AND friendlyblock=false at every
    -- position (gifts/dwarven-nature.lua:34,41,49): the signature records the
    -- allowlisted `friendlyblock=false` and the component declares the explicit
    -- friendly filter. The regular variant has neither field in its local spec, so
    -- its signature declares only `cursor_type` and it carries engine defaults.
    do
    ...
    -- The A-prime surface EXISTS and is mechanically validated: the closed
    -- stationary template, the group vocabulary, and the stationary enum members.
