...
class Acceptance(world.Acceptance):
    ...
    async def run(self,load_only=False):
        ...
        async with Client(params) as client:
            self.client=client;self.connection=await self.call('tome.connect',{});s=await self.observe();self.record('initial',s)
        ...
        async with Client(params) as client:
            self.client=client;self.connection=await self.call('tome.connect',{});loaded=await self.observe();self.record('reloaded',loaded);self.reloaded=loaded
