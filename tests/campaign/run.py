...
def compact(snapshot: dict) -> dict:
    return {k: snapshot.get(k) for k in ("phase", "control_source", "session_id", "level_instance_id",
...
class Campaign:
    ...
    async def cross_level(self) -> None:
        ...
        self.check(self.current["control_source"] == "manual", "level_change_revokes_control")
        await self.connect()
