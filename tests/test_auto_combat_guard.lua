...
do
    ...
    -- FIX1-02(b) (union, R2-APR3-01 rev5): an `act_exclude` whose native
    -- indexing would RAISE (boolean `true`/number) cannot reproduce the
    -- engine's admission; it is a typed unknown and fails closed (never
    -- permit). The diagnostic reason is the shared vocabulary's
    -- `malformed_act_exclude` (the pre-union coarse `act_exclude_not_a_table`
    -- code is superseded).
    ...
    check(malformed and malformed.action=='reject' and malformed.reason=='selffire_risk'
        ...
        and malformed.detail.reason=='malformed_act_exclude',
        'a non-indexable act_exclude is a typed unknown and fails closed (FIX1-02/R2-APR3-01)',
        malformed and malformed.detail and malformed.detail.reason)
