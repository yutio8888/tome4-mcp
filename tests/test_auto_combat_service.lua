...
do
    ...
    -- RA-05 (X-doubleprime rebase): this scenario previously mutated
    -- `controller.policy.rules` directly and cleared `policy_snapshot` to
    -- dodge the required `policy_mutated` guard, then asserted handoff/lease
    -- behaviour on that guard-disabled, non-production state. It is rebuilt
    -- through the REAL store exactly like the exact-delta adaptation above:
    -- an activated policy whose emergency rule does not match at low HP parks
    -- the emergency layer with reason `no_emergency_action`, and the
    -- handoff/owner assertions run on the transaction-validated controller.
    local svc=Service.new({host_factory=fakeHost})
    local noEmergency=policy({rules={{id='heal',priority=1,emergency=true,
        when={hp_pct={ge=90}},
        ['then']={action='use_talent',talent='T_HEALING_LIGHT',target='self'}}}})
    ...
    -- The transaction guard stays enabled end-to-end: the activated snapshot
    -- is the one the step validated against (no bypass anywhere).
