...
5. observe/inspect/status 不试运行技能，不调用 `preUseTalent`、动态 `target`、`info`、投射/命中计算来预测流程，不消耗 RNG。
...
函数来源后缀只能辅助诊断，不能证明整条调用链可靠。第三方覆盖未验证时报告兼容性未知；关键生命周期/交互入口被不兼容替换时整体禁用对应能力。不兼容组合的显式例外应带原因和回归用例，不能逐渐演变成主要技能白名单。
...
| 只读与感知 | 多次 inspect/status/候选翻页，隐藏 actor、未知地形 | RNG、资源、tick、UI 选择不变；没有额外后台信息 |
| 持久化 | 挂起时保存、人工处理后保存、合法切图自动保存、读档 | 无悬挂引用被序列化；无保存死锁；旧 ID 无效 |
| 兼容 | 旧 Python/v1、新 v2、关键入口被覆盖、原生键鼠基线 | 旧语义不被静默改变；未兼容能力诚实拒绝；普通玩法无新增钩子效果 |
