...
| 4 | `selffire` was always `"unknown"` | `Details.selffire` implements the native default: a missing `selffire` means area shapes (`ball/cone/wide`) may hit their origin (`true`), beams/hits (`beam/hit/bolt/arrow`) cannot (`false`); a function target or non-boolean value stays `"unknown"`. Used by both `act` and `inspect`. |
