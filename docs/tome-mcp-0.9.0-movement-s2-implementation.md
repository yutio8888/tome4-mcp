...
Status: **implemented on `feat/s2-ordered-queue`** (base `main@6025d93`). This
...
| `request` | one of `M.TARGET_REQUESTS` (`none`/`actor`/`grid`/`self`). |
...
`target_requests` is omitted it is **derived** from the sequence. Existing
...
| --- | --- | --- |
| `< 4` | absent | `{'none'}` (`self_random_teleport`) |
...
| Condition | Behaviour |
| --- | --- |
| Extra, reordered/wrong-kind or non-optional missing prompt | `unexpected_target_request` with `expected={index,request}` / `observed={index,request|nil}` / `skippable`; pause, never resubmit, interaction handed back. |
...
| A decided value cannot be evaluated (unresolvable subject actor, no planned destination) | `movement_request_value_unknown` with `{index,request,dependency}`; pause before answering with a wrong value. |
...
the client-visible policy log as a `paused`/`denied` detail.
...
- **Lua**: `bash tests/run.sh` (42 suites; includes the new
  `tests/test_auto_combat_sequence.lua`, 69 checks) and the updated
  `test_effect_manifest.lua` / `test_auto_combat_movement_factory.lua` /
  `test_auto_combat_movement.lua` / `test_runtime.lua`.
