...
> **历史资料，非规范（Historical / non-normative）。** 本文是 0.4.0 的交付记录。其中“成长查询不运行
> `canLearnTalent`、技能需求函数、动态说明或 clone；物品查询不运行鉴定、命名或 tooltip 方法……未知或
> 修改后的实现明确返回未知/不支持”体现的是已被 `AGENTS.md` 与 `docs/tome-mcp-auto-combat-plugin-design.md`
> §8.3 **取代**的读取纯度/身份前提。当前：读取仅受两条红线约束（不提交动作、不泄露玩家未知信息），
> 当前实时的动态 getter/需求函数可调用（允许 RNG/读副作用），报错/缺失/`nil` 时标 `unknown`；
> 第三方替换本身不作为拒绝理由。历史观测与哈希保留作证据。
...
成长查询不提交任何动作、不泄露玩家未知信息（这是读取的两条红线）；它可调用当前实时的 `canLearnTalent`、技能需求函数、动态说明等 getter 求值（允许 RNG/读副作用），不可得时标 `unknown`；动态说明文本当前不作为字段暴露属于**数据模型/能力取舍**。物品查询不执行物品效果（动作边界），当前也不通过 tooltip/鉴定方法取信息（能力取舍），但不因此推断“不得调用”。需求通过当前实时规则和现存字段读取，未知/不可得的实现返回未知/不支持，执行时重新进行原生检查。地面读取受当前视野、感知、失明及隐藏条件约束；不提供未见世界物品列表。
