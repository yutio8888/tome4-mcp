...
状态：本批11项已完成独立验收；原始13项中的SYS-08/09及调查项U-01仍明确延期。最终源码084642634b1d92dfb5e2880f43998672469798ab；包3a4e186b001971bdebfa9315a33dcac3ddd06c9abd7a1698c9af6caf03f413fb。协调者据用户指示处理合并。

| ID | 问题 | 批次 / owner | 状态 | 验收要求 |
| --- | --- | --- | --- | --- |
| SYS-01 | 连续动作上限 | policy Dev | FIXED_VERIFIED | 精确额度/请求数、pause/resume/pending/sustain、source+dist native |
| SYS-02 | emergency_only fallback | policy Dev | FIXED_VERIFIED | 两种显式模式、无活锁、旧策略可见迁移、source+dist native |
| SYS-03 | 预算契约分叉 | policy Dev | FIXED_VERIFIED | 双计数、跨pump界、规范统一、source+dist native |
| SYS-04 | local clear 持久化 | core Dev | FIXED_VERIFIED | local/remote对称、reset/reload、原生保存证据 |
| SYS-05 | 边界 checker 缺失 | tooling Dev | FIXED_VERIFIED | checker负例与tests/run接线；候选结合SYS-12后统一验证 |
| SYS-06 | manifest 悬空证据 | tooling Dev | FIXED_VERIFIED | dangling/duplicate/missing/hash负例及合法历史格式兼容 |
| SYS-07 | 候选证据归档 | 协调者，集成候选阶段 | FIXED_VERIFIED | 最终source/dist身份、可取得raw/hash索引；依赖前三批 |
...
| SYS-10 | README版本/能力漂移 | core Dev | FIXED_VERIFIED | v4/工具列表/当前能力边界与实际schema一致 |
| SYS-11 | public act接受internal字段 | core Dev | FIXED_VERIFIED | onRequest/JSON生产边界未受理/未排队、合法internal回归 |
| SYS-12 | 入站闭合/数组校验 | core Dev | FIXED_VERIFIED | schema与实际解码一致、精确无动作副作用 |
| SYS-13 | dismiss错误示例 | core Dev | FIXED_VERIFIED | 工具描述示例通过实际Pydantic/interaction契约 |
| U-01 | max_candidates消费未定 | policy Dev取证，协调者定稿 | REPRODUCED / DEFERRED | 已复现未消费；本批不任意截断，后续语义与owner见同日期todo文档；不得称已修 |

FIXED_VERIFIED 仅覆盖表中11项；SYS-08/09/U-01的延期不等于完成。历史派发/中间状态保留在下文，最终裁决以本文件末尾及acceptance.json为准。
...
| RUNTIME-REV-01 | P2：MCP scalar 与 v4 类型/字节/范围约束双向不齐（包括错误拒绝合法长ID） | core；Revision已交付79cf8c4，完整共享标量与实际MCP矩阵待最终提交 | CLOSED：core4bc0bb2，独立47测试/1,787MCP边界及source/dist通过 |
| RUNTIME-REV-02 / NATIVE-PENDING-01 | P2：真实 rest 已完成，结算停机后同帧 pump 清掉 stopped controller | core 5fb875b + policy c7a7827；真实 tick/display 回归 | CLOSED：source/dist各20项与独立复核通过 |
| RUNTIME-REV-03 | P2：双 legacy draft/approved 迁移丢失原 draft 表示 | policy ae8baf8；完整原始 canonical bytes 经 load/save/reload | CLOSED：source/dist最终20项及独立复核通过 |
| RUNTIME-REV-04 | P2：导入 envelope 缺失或非字符串 hash 绕过原始哈希验证 | policy dca36dc；结构有效后保留既有语义校验顺序 | CLOSED：source/dist最终20项及独立复核通过 |
| EVIDENCE-REV-01 | P1：denseArray 与字段复制放在死分支，checker 仍 PASS | tooling；原 Reviewer 两个 scratch 负例与作用域注册 | CLOSED：原独立Reviewer负例复核通过 |
| EVIDENCE-REV-02 | P1：复制后 no_restrict=nil 未被 checker/guard 回归捕获 | tooling checker + policy 15字段真实转发 oracle | CLOSED：原独立Reviewer负例复核通过 |
| NATIVE-IDENTITY-01 | 夹具错误比较跨进程临时 UID；引擎加载会分配新 UID | core 5fb875b；改为 puuid/save_name、approved bytes、保存副本哈希及错误身份负例 | CLOSED：source/dist各123项及独立身份/存档复核通过 |
...
- Runtime Reviewer `137119c2-30bc-4f4d-9a4d-7762c9163a6f` ACCEPT：SYS01/02/03/04/10/11/12/13；RUNTIME-REV01/02/03/04全部CLOSED。
- Evidence Reviewer `de482ffb-434f-468a-8c18-1ff3997e90c5` ACCEPT：SYS05/06/07；EVIDENCE-REV01/02全部CLOSED。
