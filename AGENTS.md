...
- **不得施加策略性限制**：凡是已审计、可执行的游戏操作（移动、撤退/风筝、位移/传送、
...
- **只对插件自身完整性/不可判定性 fail-closed**：目标丢失或不可解析、getter 不可审计、源漂移、
  控制权丢失、预算/租约边界、原生拒绝——这些表示“插件无法正确执行”，而**非**“策略不好”。
  **已知风险**（如自伤概率）默认只报告，由策略的阈值决定是否拒绝。
