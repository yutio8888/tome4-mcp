...
优先使用已存在的感知缓存；不会为观察额外调用 `canSee`、随机函数或技能预检。原生实现清空缓存后，对位于当前视野且无致盲／隐形／潜行／隐匿条件的普通角色使用确定性判断；无法安全判断时省略角色。桥接调用当前实时的原生可见性方法并校验其返回值；第三方替换方法本身只是**遥测/重审提示**，不是隐藏角色可见性的理由，只有方法缺失/报错/返回 `nil`/无效时才省略。地形记忆从桥接实际看见的内容累积。未适配成本标注为未知或基础值，不保证实际施法成功。
