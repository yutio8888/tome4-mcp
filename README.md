...
已归属的 `change_level` 跨场景后保留当前对话和原命令，但控制租约失效：显式重新 `connect`，再 `status` 原 command_id 并回答，不能再次发起换层。原生加载等待窗口由游戏自动关闭；自动保存延后到这次交互全部结束。其他未声明的场景切换、未知 UI 仍交给玩家（第三方替换实现本身只是遥测/重审提示，不是交给玩家的理由）。
...
`known=true` 表示本桥接会话曾观察过，`visible=true` 表示当前可见。原生 `remembers`、全图照明和已发现地点列表不会自动导入。视野外保留已有桥接记忆，不读取隐藏变化；重启游戏后重新积累。`observe` 的局部地图范围最多为身周 12 格；`tome.map` 另提供玩家已知的地图行或指定区域。`is_exit` 标识可见通路，`name` 为其可见名称；不会返回隐藏目的地数据。`block_status` 只表示地形通行规则，实际移动和进入仍由原生命令判定。
...
`dialog.notice` 支持原生 QuestPopup、LorePopup、ShowLore，以及可关闭的 simplePopup / simpleLongPopup。只显示实际栈顶；每次回答关闭一层，可能随后出现下一层。原生回合末回调继承发起动作的归属。不要为关闭弹窗再次攻击、移动或拾取；去重、断线恢复、人工接管和 Battle Companion 互斥覆盖整个过程。已归属换层中的原生加载等待弹窗由游戏自动完成；其他无关闭入口的等待弹窗或未知类仍交给玩家。桥接调用当前实时的原生回调并校验其返回值；第三方替换回调本身只是**遥测/重审提示**，绝不是交给玩家的理由，只有回调缺失/报错/返回 `nil`/无效或交互无法解析时才交给玩家。
...
已鉴定物品的 `activation` 提供存储中的充能等字段；`present` 表示存在激活定义，`runtime_checked` 表示仍须原生执行检查。查询**不提交物品效果**（那属于动作边界）；动态说明文本目前不作为字段暴露，这是**数据模型/能力取舍**（不是纯度门槛），不排除未来调用实时 getter。使用后的持续效果按普通游戏回合运行；例如 Rod of Recall 激活完成后还需要原生回合才能传送。
...
原生协程不进入存档：挂起期间的保存会推迟到本次调用解决后。无法恢复的原生错误会保留部分结果、标记 `uncertain`，并限制写入直到重新加载。能力列表的 `activation.admitted` 表示允许尝试原生入口，不保证资源、距离或后续界面都满足；具体输入在运行中验证。若所需实时值不可得（缺失/报错/返回 `nil`/类型无效）或原生交互无法解析，桥接拒绝该自动执行或交给玩家；第三方插件替换了相关原生入口本身只是**遥测/重审提示**，不是拒绝条件。
...
优先使用已存在的感知缓存；不会为观察额外调用 `canSee`、随机函数或技能预检。缓存未命中时，对位于当前视野且无致盲／隐形／潜行／隐匿条件的普通角色使用确定性、不回调的纯标量投影来判断；无法安全判断时省略角色。该投影不做任何来源身份校验：第三方替换的感知方法**既不构成门槛，也不是隐藏角色的理由**。地形记忆从桥接实际看见的内容累积。未适配成本标注为未知或基础值，不保证实际施法成功。
...
等级经验、点数、物品和可见敌人检查使用现存字段（快照的**数据模型**选择）。未知物品保持未鉴定；基础攻防和抗性不等于最终战斗公式。快照不为填充这些字段而运行物品鉴定、动态说明或战斗预检；这只是字段能力取舍，不代表读取路径禁用实时 getter（见上）。
