...
def check_schema_envelope(protocol: Path) -> list[str]:
    ...
    return [f"requests: {len(schema_ops)} live ops, {len(requests['$defs']['Args']['oneOf'])} args unions, v=4"]
...
def check_generated(protocol: Path, errors: dict) -> list[str]:
    for path, expected in generated_modules(protocol, errors).items():
        if not path.is_file() or path.read_text() != expected:
            raise Failure(f"generated error registry out of date: {path}")
    return [f"errors: generated Lua/Python registries up to date ({len(errors['codes'])} codes)"]
